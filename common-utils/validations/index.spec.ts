...
import { getPhoneNumberPattern, getEmailPattern, getIPPattern } from '.';
...
describe('Pattern functions', () => {
  test('getPhoneNumberPattern should return correct pattern and message', () => {
    const pattern = getPhoneNumberPattern();
    expect(pattern.value.test('010-1234-5678')).toBeTruthy();
    expect(pattern.message).toBe(
      '올바른 핸드폰 번호 형식을 입력해주세요 (ex: 010-1234-5678)'
    );
  });

  test('getEmailPattern should return correct pattern and message', () => {
    const pattern = getEmailPattern();
    expect(pattern.value.test('test@example.com')).toBeTruthy();
    expect(pattern.message).toBe('올바른 이메일 형식을 입력해주세요');
  });

  test('getIPPattern should return correct pattern and message', () => {
    const pattern = getIPPattern();
    expect(pattern.value.test('192.168.0.1')).toBeTruthy();
    expect(pattern.message).toBe('올바른 IP 형식을 입력해주세요');
  });
});
