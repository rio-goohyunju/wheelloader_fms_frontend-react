...
import usePreSignedUrl from '../common/usePreSignedUrl';
...
const getToastBackgroundColor = (level: string, opacity?: Opacitys) => {
  if (level.toLowerCase().includes('critical'))
    return `${colors.red[200]}${opacity ?? ''}`;
  if (level.toLowerCase().includes('warning'))
    return `${colors.pastelOrange[50]}${opacity ?? ''}`;
  return 'white';
};
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Typography variant="h6" color={getToastTextColor(level)}>
          발생 시간 :
          {dayjs.unix(eventDate).format('YYYY년MM월DD일 HH시mm분ss초')}
        </Typography>
      </CardContent>
      <CardMedia
        component="img"
