...
export const fetchSchedules = (params: FetchParams) => {
  return client.get<FetchScheduleResponse[]>(
    `/schedule?site_id=${params.siteID}&status=SCHEDULE`
  );
};
...
export const fetchBlocker = (params: FetchBlockerParams) => {
  return client.get<FetchBlockerResponse>(
    `robot/blocker/${params.robotID}/?site_id=${params.siteID}`
  );
};
