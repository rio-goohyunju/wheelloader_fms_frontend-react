...
export const fetchAlertHistories = (variable: FetchAlertHistoriesParams) => {
  ...
  return client.get<FetchAlertHistoriesResponse>(
    `/alert-history?${params.toString()}`
  );
};

export const fetchAlertHistoryReport = (
  variable: FetchAlertHistoryReportParams
) => {
  return client.get(
    `/alert-history/detail/${variable.uid}?site_id=${variable.siteID}`
  );
};
